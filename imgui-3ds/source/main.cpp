...
int main(int argc, char* argv[])
{
   ...
   // C3D_TexSetFilter(image.tex, GPU_LINEAR, GPU_LINEAR);
	...
	/* we render the ImGui output on the bottom screen; the variable name used
	   to be `top` which was misleading. */
	C3D_RenderTarget* bottom = C2D_CreateScreenTarget(GFX_BOTTOM, GFX_LEFT);
	consoleInit(GFX_TOP, NULL); /* keep the console on the top screen */
	printf("Ho!\n");
	...
	while (aptMainLoop()) 
	{
		...
		// ensure CPU writes are visible to GPU before we draw the texture
		// (debug: print a message so we can see this code path is executed)
		printf("[imgui-sample] flushing texture at %p\n", image.tex->data);
		GSPGPU_FlushDataCache(image.tex->data, 512 * 256 * 4);
		// draw to the bottom screen
