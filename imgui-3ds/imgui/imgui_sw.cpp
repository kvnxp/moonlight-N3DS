...
// The global math-operators defined by ImGui (inside imgui_internal when
// IMGUI_DEFINE_MATH_OPERATORS is set) cover some common cases, but not the
// scalar-first multiplication or inequality.  We always provide those
// ourselves.  Addition operators conflict with the global versions, so we
// only define them when the macro hasn't been enabled yet.

// scalar * vector helpers – not provided by ImGui's default operators
...
// If the global math operators aren't defined, also provide the
// vector+vector overloads that would otherwise collide.
