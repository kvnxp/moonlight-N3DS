...
# build helper libraries if not already present
# helper scripts install static libs to ${DEVKITPRO:-}/portlibs/3ds
# (DEVKITPRO should be set by the environment setup above).  We run them
# unconditionally so that they work even when PORTLIBS isn’t defined.
for script in "$(dirname "$0")"/3ds/build-libpng.sh \
              "$(dirname "$0")"/3ds/build-freetype.sh; do
    if [ -f "$script" ]; then
        echo "===> running $script"
        bash "$script"
    fi
